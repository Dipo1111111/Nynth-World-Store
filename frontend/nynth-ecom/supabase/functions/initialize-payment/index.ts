...
Deno.serve(async (req) => {
  ...
  const res = await fetch("https://api.paystack.co/transaction/initialize", { method: "POST", headers: { Authorization: "Bearer " + secret, "Content-Type": "application/json" }, body: JSON.stringify({ amount: Math.round(Number(amount) * 100), email, metadata, callback_url: (req.headers.get("origin") || "https://nynth.com") + "/thank-you" }) });
