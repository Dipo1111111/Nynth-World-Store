...
- **Events admin page (2026-09-24, uncommitted):** new `/admin/events` (nav: Events) with per-event sold/left/checked-in/revenue, status chips, capacity bar, hide/show toggle, view/check-in/edit shortcuts. Stats in `src/utils/eventStats.js` + tests. Test orders excluded. Gate: lint 0, 70 Vitest, build, verify-dist.
